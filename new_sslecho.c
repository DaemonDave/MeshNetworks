 ...
 */
//! \note this is plexus 1.0 - a leveraged start from OpenSSL echo
...
//! \fn CreateKeyTable creates a file database for KEY SSL encryptions for server side
void CreateKeyTable( const char * filename )
{
   sqlite3 *db;
   char *zErrMsg = 0;
   int rc;
   ...
   sql =  "CREATE TABLE key (Id INT, Data BLOB);";
...
//! \fn CreateCertTable creates a file database for PEM SSL encryptions for client side
void CreateCertTable( const char * filename )
{
   sqlite3 *db;
   char *zErrMsg = 0;
   int rc;
   ...
   // Create SQL statement table to instantiate a database
...
//! \fn CheckFileStatus checks raw and db files for existence and size as a simple measure of whether or not it makes sense to create a new one (which may or may not fail later on)
int CheckFileStatus( const char * filename)
{
	struct stat sb;
	if (lstat(filename, &sb) == -1)
		perror("lstat failed");
    if ( sb.st_size > FILEMAX  &&  ((sb.st_mode & S_IFMT) == S_IFREG))// SQLITE3 DB files exist at least at 8KB so a rough guess and it's a regular file by mode integer resolution
    {
		return 0;
	}
	else return -1;
}
//! \fn LoadPem loads database PEM SSL encryptions for client side \var filename is the .pem file and \var dbfilename is the database filename to store to 
int LoadCert( const char * filename, const char * dbfilename) 
{
   ...
   if ( !CheckFileStatus( filename) )
	CreateCertTable( filename ); // Second create the database and see error if fails....
   //  Basic Algorithm
   ///----- file side
   ...
   fseek(fp, 0, SEEK_END);
	...
	printf( "processed certificate : %lld \n", (long long) strlen(data) );  
    if (ferror(fp)) 
    {
...
/**   BOILERPLATE OPENSSL SOCKET FUNCTIONS
 * 
 *   DO NOT TOUCH UNLESS A DESIGN REPLACES FUNCTIONALITY
 * 
 * */
...
//! \note Configure Functions that insert new behaviour to the system

void configure_server_context(SSL_CTX *ctx)
{
    /* Set the key and cert */
	/// NEW LOGIC - load into certificate database at the same time for perusal and db testing
	if ( !LoadCert( "cert.pem", "pem.db") )
	{
		;// do nothing for now
	} 
	else // fail loudly
		perror(" LoadCert failed __FILE__  __LINE__ ...");     
...
{
	/// NEW LOGIC - load into key database at the same time for perusal and db testing
	if ( !LoadCert( "cert.pem", "pem.db") )
	{
		;// do nothing for now
	} 
...
/**   BOILERPLATE OPENSSL SOCKET SERVER / CLIENT MAIN
 * 
 *   DO NOT TOUCH UNLESS A DESIGN REPLACES FUNCTIONALITY
 * 
 * */

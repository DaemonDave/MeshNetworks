...
//! \note moving compile order causes these header configs to stop working...
#define _POSIX	1
...
// DRE 2022
//! \note added for Linux compatibility
#include <sys/socket.h>
#include <netinet/in.h>
#include <sys/vfs.h>    /* or <sys/statfs.h> */
#include <fcntl.h>
#include <semaphore.h> // for sem_init   sem_t
#include <stdlib.h> 	// for =ERANGE
#include <unistd.h>		// for pipe
// weird Intel debug format
//
//! \ reuse fixes
...
extern char ILibScratchPad[4096];
...
struct packetheader
{
	...
	*/
	...
	/*! \var ReceivingAddress
		...
		\a ILibWebClient.
	*/
	char* ReceivingAddress[32];
	int ReceivingAddressSize;
...
//
#define ILIBCRITICALEXIT(ex) {ILibCriticalLog(NULL, __FILE__, __LINE__, ex, 0);exit(ex);}
#define ILIBCRITICALEXIT2(ex,u) {ILibCriticalLog(NULL, __FILE__, __LINE__, ex, u);exit(ex);}
#define ILIBCRITICALEXIT3(ex,m,u) {ILibCriticalLog(m, __FILE__, __LINE__, ex, u);exit(ex);}

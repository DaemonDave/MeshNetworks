...
//! UGH mystery numbers and out of range printfs... 
#define ONEHUNDREDCHARS 100
...
int ILibGetLocalIPv4AddressList(struct sockaddr_in** addresslist)
{
#ifdef WINSOCK2
    DWORD i, j = 0;
    SOCKET sock;
    ...
    if (WSAIoctl(sock, SIO_GET_INTERFACE_LIST, NULL, 0, &interfaces, 32 * sizeof(INTERFACE_INFO), &interfaces_count, NULL, NULL) == SOCKET_ERROR || interfaces_count == 0)
    {
        *addresslist = NULL;
        closesocket(sock);
        return 0;
    ...
    {
        if (interfaces[i].iiAddress.AddressIn.sin_addr.S_un.S_addr != 0x0100007F) j++;
    }

    // Allocate the IPv4 list and copy the elements into it
    if ((*addresslist = malloc(sizeof(struct sockaddr_in) * j)) == NULL) ILIBCRITICALEXIT(254);
    j = 0;
    for (i = 0; i < interfaces_count; i++)
    {
        if (interfaces[i].iiAddress.AddressIn.sin_addr.S_un.S_addr != 0x0100007F)
        {
    ...
    int ctr=0;
    int i;
    *addresslist = malloc(sizeof(struct sockaddr_in) * 16); // TODO: Support more than 16 adapters
...
int ILibGetLocalIPv6IndexList(int** indexlist)
{
    ...
    {
        // Skip any loopback adapters
        if (pAddressesPtr->PhysicalAddressLength != 0 && pAddressesPtr->NoMulticast == 0 && pAddressesPtr->OperStatus == IfOperStatusUp)
        {
            // For each adapter
            AddrCount++;
        }
    ...
    // Allocate the array of IPv6 addresses
    *indexlist = ptr = malloc(AddrCount * sizeof(int));
...
//! it the chain of network "connections" that this agent is "chained to".
...
void ILibStartChain(void *Chain)
{
    ...
    //
    c = (struct ILibBaseChain*)Chain;
    c = c->Next; // Skip the base timer which is the first element in the chain.
    while (c != NULL && c->Object != NULL)
    {
        //
...
void ILibHashTree_GetValue(void *tree_enumerator, char **key, int *keyLength, void **data)
{
    ...
    if (key!=NULL) 
    {
        *key = en->node->KeyValue;
    }
    if (keyLength!=NULL) 
    {
        *keyLength = en->node->KeyLength;
    }
    if (data!=NULL) 
    {
        *data = en->node->Data;
...
void ILibDestructPacket(struct packetheader *packet)
{
    ...
    if (packet->UserAllocStrings != 0)
    {
        ...
        {
            free(packet->StatusData);
        }
        if (packet->Directive != NULL) 
        {
            free(packet->Directive);
        }
        if (packet->Reserved == NULL && packet->DirectiveObj != NULL) 
        {
            free(packet->DirectiveObj);
        }
        if (packet->Reserved != NULL) 
        {
            free(packet->Reserved);
        }
...
SOCKET ILibGetSocket(struct sockaddr *localif, int type, int protocol)
{
    int off = 0;
    SOCKET sock;
    if (localif->sa_family == AF_INET6 && g_ILibDetectIPv6Support == 0) 
    {
        PRINTERROR();
        return 0;
    }
    if ((sock = socket(localif->sa_family, type, protocol)) == -1) 
    {
        return 0;
    }
    if (localif->sa_family == AF_INET6) setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&off, sizeof(off));
#if defined(WIN32)
...
void ILibParseUri(char* URI, char** IP, unsigned short* Port, char** Path)
{
    ...
    if ((*IP = (char*)malloc(TempStringLength2 + 1)) == NULL) ILIBCRITICALEXIT(254);
...
struct packetheader* ILibClonePacket(struct packetheader *packet)
{
    ...
    // Copy the addresses
    memcpy(&RetVal->ReceivingAddress, &packet->ReceivingAddress, packet->ReceivingAddressSize),
           RetVal->ReceivingAddressSize = packet->ReceivingAddressSize;
    memcpy(&RetVal->Source, &packet->Source, packet->SourceSize),
           RetVal->SourceSize = packet->SourceSize;
...
void ILibLifeTime_AddEx(void *LifetimeMonitorObject,void *data, int ms, ILibLifeTime_OnCallback Callback, ILibLifeTime_OnCallback Destroy)
{
    ...
    ILibLinkedList_Lock(UPnPLifeTime->ObjectList);// printf("[L1]");
    ...
    ILibLinkedList_UnLock(UPnPLifeTime->ObjectList);// printf("[U1]");
}
...
void ILibLifeTime_Check(void *LifeTimeMonitorObject, void *readset, void *writeset, void *errorset, int* blocktime)
{
    ...
    while (node != NULL)
    {
        ...
        else
        {
            ...
            //
            if (EVT->DestroyPtr != NULL) 
            {
                EVT->DestroyPtr(EVT->data);
            }
        }
...
char* ILibTime_Serialize(time_t timeVal)
{
    struct tm *T;
    char *RetVal;
    if ((RetVal = (char*)malloc(ONEHUNDREDCHARS)) == NULL) ILIBCRITICALEXIT(254);
    ...
    snprintf(RetVal, ONEHUNDREDCHARS, "%04d-%02d-%02dT%02d:%02d:%02d",
             T->tm_year + 1900,
             T->tm_mon + 1,
             T->tm_mday,
...
int ILibIsLoopback(struct sockaddr *addr)
{
    ...
    {
        if (memcmp(&((struct sockaddr_in*)addr)->sin_addr, IPv4Loopback, 4) == 0) return 1;
        else return 0;
    }
...
int ILibInet_pton(int af, const char *src, char *dst)
{
    ...
    else
    {
        ...
        sscanf_s(src, "%d.%d.%d.%d", (unsigned char*)(dst), (unsigned char*)(dst + 1), (unsigned char*)(dst + 2), (unsigned char*)(dst+ 3));
        return 1;
    }
}
...
void ILibCriticalLog(char* msg, char* file, int line, int user1, int user2)
{
    ...
    len = snprintf(ILibScratchPad, 4096, "\r\n%s:%d (%d,%d) ", file, line, user1, user2);
    ILibAppendStringToDiskEx("C:\\CriticalLog.txt", ILibScratchPad, len);

    if (msg != NULL)
    {
#ifdef WIN32
        ILibAppendStringToDiskEx("C:\\CriticalLog.txt", msg, strlen(msg));
#else
        ILibAppendStringToDiskEx("criticallog.txt", msg, strlen(msg));
#endif
    }
}

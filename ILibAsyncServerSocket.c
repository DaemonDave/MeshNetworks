...
ILibAsyncServerSocket_ServerModule ILibCreateAsyncServerSocketModule(void *Chain, int MaxConnections, unsigned short PortNumber, int initialBufferSize, int loopbackFlag, ILibAsyncServerSocket_OnConnect OnConnect, ILibAsyncServerSocket_OnDisconnect OnDisconnect, ILibAsyncServerSocket_OnReceive OnReceive, ILibAsyncServerSocket_OnInterrupt OnInterrupt, ILibAsyncServerSocket_OnSendOK OnSendOK)
{
    ...
    if (ILibDetectIPv6Support())
    {
        ...
        localif.sin6_addr = (loopbackFlag == 1?in6addr_loopback:in6addr_any);
...
#ifdef WINSOCK2
        ((struct sockaddr_in*)&localif)->sin_addr.S_un.S_addr = htonl((loopbackFlag == 1?INADDR_LOOPBACK:INADDR_ANY));
#else
        ((struct sockaddr_in*)&localif)->sin_addr.s_addr = htonl((loopbackFlag == 1?INADDR_LOOPBACK:INADDR_ANY));
#endif
    ...
    if ((RetVal->ListenSocket = socket(localif.sin6_family, SOCK_STREAM, IPPROTO_TCP)) == -1) 
    {
        return 0;
    }

    // Setup the IPv6 & IPv4 support on same socket
    if (localif.sin6_family == AF_INET6) setsockopt(RetVal->ListenSocket, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&off, sizeof(off));

    // Setting the re-use on a TCP socket allows reuse of the socket even in timeout state.
    setsockopt(RetVal->ListenSocket, SOL_SOCKET, SO_REUSEADDR, (char*)&ra, sizeof(ra));

    // Bind the socket
#if defined(WIN32)
    if (bind(RetVal->ListenSocket, (struct sockaddr*)&localif, INET_SOCKADDR_LENGTH(localif.sin6_family)) != 0) {
